...
| 1.2 | Активный таб НЕ приостанавливается, даже если истёк таймаут | P0 | Unit | ⚠️ | `ActiveDisabled.test.ts` частично |
| 1.3 | При активации таба счётчик времени сбрасывается в 0 | P0 | Unit | ❌ | |
| 1.4 | Таб с `status !== 'complete'` не приостанавливается | P1 | Unit | ❌ | |
| 1.5 | Расширение выключено (`active=false`) → ни один таб не приостанавливается | P1 | Unit | ✅ | `ActiveDisabled.test.ts` |
| 1.6 | Пауза (`pauseTics > 0`) предотвращает приостановку | P1 | Unit | ❌ | |
| 1.7 | По истечении паузы приостановка возобновляется | P1 | Unit | ❌ | |
| 1.8 | Адаптивный таймаут увеличивается с ростом частоты посещений | P2 | E2E | ✅ | `adaptive-timeout.test.ts` |
| 1.9 | Приостановка идемпотентна — повторный вызов не ломает состояние | P1 | Unit | ❌ | |
| 1.10 | Таб закрыт во время процесса приостановки — ошибок не должно быть | P1 | Unit | ❌ | Race condition |
...
| 2.5 | После восстановления флаги `parked`, `time`, `suspended_time` сброшены | P1 | Unit | ❌ | |
| 2.6 | Hover на иконку восстанавливает таб (`restoreOnMouseHover=true`) | P2 | E2E | ❌ | |
| 2.7 | Повторное восстановление уже восстановленного таба не вызывает ошибок | P1 | Unit | ❌ | |
| 2.8 | Формы заполнены до приостановки → данные восстановлены после | P1 | E2E | ✅ | `form-data-restore.test.ts` |
| 2.9 | YouTube: временна́я метка видео сохранена и восстановлена в URL | P2 | E2E | ❌ | `?t=seconds` |
| 2.10 | Массовое восстановление всех табов окна — с задержками между восстановлениями | P2 | E2E | ❌ | |
...
|-----------|-------|-----------|------------|--------------|
| P0 (критично) | 13 | 11 | 1 | 1 |
| P1 (высокий) | 47 | 31 | 0 | 16 |
| P2 (средний) | 40 | 15 | 1 | 24 |
| P3 (низкий) | 3 | 0 | 0 | 3 |
| **Итого** | **103** | **57 (55%)** | **2 (2%)** | **44 (43%)** |
